    ...
    <el-card>
      ...
      >
        ...
        <el-table-column label="统计" width="120">
          <template #default="{ row }">
            <div class="post-stats">
              <span
                ><el-icon><View /></el-icon> {{ row.views || 0 }}</span
              >
              <span
                ><el-icon><ChatDotRound /></el-icon> {{ row.comments_count || 0 }}</span
              >
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="editPost(row.id)"> 编辑 </el-button>
            <el-button link type="primary" size="small" @click="viewPost(row.id)"> 预览 </el-button>
...
async function batchDraft() {
  try {
    await ElMessageBox.confirm(
      `确定要将选中的 ${selectedPosts.value.length} 篇文章转为草稿吗?`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
async function batchDelete() {
  try {
    ...
    loading.value = true;
    const promises = selectedPosts.value.map(post => postsAPI.deletePost(post.id));
    await Promise.all(promises);
